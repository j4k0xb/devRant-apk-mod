...
function isBoxedPrimitive(value) {
  ...
  return (
    ...
    isBooleanObject(value) || // || isBigIntObject(value)
...
E(
  ...
  (message) => {
    const suffix =
      "This is caused by either a bug in Titanium " +
...
function isStackOverflowError(err) {
  ...
  return (
    err.name === maxStack_ErrorName && err.message === maxStack_ErrorMessage
  );
}
...
class SlowBuffer {
  ...
  static fromTiBuffer(tiBuffer, start, length) {
    return new Proxy(
      new SlowBuffer(tiBuffer, start, length),
      arrayIndexHandler
    ); // eslint-disable-line security/detect-new-buffer
  }
 ...
 */
const { ALL_PROPERTIES: ALL_PROPERTIES$1, ONLY_ENUMERABLE: ONLY_ENUMERABLE$1 } =
  propertyFilter;
  ...
  if (typeof arg !== "object" || arg.apiName !== "Ti.Buffer") {
    ...
    if (typeof arg === "number") {
      if (typeof encodingOrOffset === "string") {
        throw new TypeError(
          `The "string" argument must be of type "string". Received type ${typeof arg}`
        );
      }
...
Buffer$1.from = function (value, encodingOrOffset, length) {
  ...
  throw new TypeError(
    "The 'value' argument must be one of type: 'string', 'Array', 'Buffer', 'Ti.Buffer'"
  );
};
/**
...
Buffer$1.prototype.compare = function (
  target,
  targetStart,
  targetEnd,
  sourceStart,
  ...
  if (!Buffer$1.isBuffer(target)) {
    throw new TypeError(
      `The "target" argument must be one of type Buffer or Uint8Array. Received type ${typeof buf1}`
    );
  }
...
const TypedArrayProto_byteLength = Object.getOwnPropertyDescriptor(
...
Buffer$1.compare = function (buf1, buf2) {
  if (!Buffer$1.isBuffer(buf1)) {
    throw new TypeError(
      `The "buf1" argument must be one of type Buffer or Uint8Array. Received type ${typeof buf1}`
    );
...
Buffer$1.isBuffer = function (obj) {
  return (
    obj !== null &&
    obj !== undefined &&
    (obj instanceof Buffer$1 || obj[isBuffer] === true)
  );
};
...
Buffer$1.prototype[customInspectSymbol] = function (recurseTimes, ctx) {
  ...
  let str = this.slice(0, actualMax)
    .toString("hex")
    .replace(/(.{2})/g, "$1 ")
    .trim();
...
function checkOffset(buffer, offset, byteLength) {
  ...
  if (offset < 0 || offset > endOffset) {
    throw new RangeError(
      `The value of "offset" is out of range. It must be >= 0 and <= ${endOffset}. Received ${offset}`
    );
  }
}
/**
...
// Copyright Node.js contributors. All rights reserved.
const { ALL_PROPERTIES: ALL_PROPERTIES$2, ONLY_ENUMERABLE: ONLY_ENUMERABLE$2 } =
  propertyFilter;
...
function formatValue(ctx, value, recurseTimes, typedArray) {
  ...
  if (ctx.customInspect) {
    ...
    if (
      typeof maybeCustom === "function" && // Filter out the util module, its inspect function is special.
          ...
          if (sourceMatch) {
            ...
            if (filePath.startsWith("file:")) {
              filePath = filePath.replace(
                `file://${Ti.Filesystem.resourcesDirectory}`,
                ""
              );
            }
...
class Console {
  ...
  _writeToConsole(level, string) {
    ...
    if (this._apiModule) {
      ...
      const useStdErr =
        level === "warn" || level === "error" || level === "trace";
  ...
  info(...args) {
    this._writeToConsole(
      "info",
      formatWithOptions(kColorInspectOptions, ...args)
    );
  }
  ...
  warn(...args) {
    this._writeToConsole(
      "warn",
      formatWithOptions(kNoColorInspectOptions, ...args)
    );
  }
  ...
  groupEnd() {
    this._groupIndent = this._groupIndent.slice(
      0,
      this._groupIndent.length - 2
    );
  }
  ...
  table() {
    if (!tableWarned) {
      tableWarned = true;
      process.emitWarning(
        '"console.table" is not yet implemented in Titanium!'
      );
    }
  }
}
...
/* globals OS_ANDROID */
{
  Date.prototype.toLocaleDateString = function () {
    const properties =
      Intl.DateTimeFormat._makeTiCreationPropertiesFrom(arguments);
...
function makeTiFormatCreationPropertiesFrom(
  args,
  supportedFormatLocalesFunction
) {
  const properties = {};
...

{
  ...
  function TiCollator() {
    const properties = makeTiFormatCreationPropertiesFrom(
      arguments,
      Ti.Locale.getSupportedCollatorLocales
    );
  ...
  function TiDateTimeFormat() {
    const properties = makeTiFormatCreationPropertiesFrom(
      arguments,
      Ti.Locale.getSupportedDateTimeFormatLocales
    );
    return new Ti.Locale.DateTimeFormat(properties);
  }

  TiDateTimeFormat._makeTiCreationPropertiesFrom = (args) => {
    return makeTiFormatCreationPropertiesFrom(
      args,
      Ti.Locale.getSupportedDateTimeFormatLocales
    );
  };

  TiDateTimeFormat.supportedLocalesOf =
    Ti.Locale.getSupportedDateTimeFormatLocales; // Set up an "Intl.NumberFormat" type which wraps our undocumented "Ti.Locale.NumberFormat" proxy.

  function TiNumberFormat() {
    const properties = makeTiFormatCreationPropertiesFrom(
      arguments,
      Ti.Locale.getSupportedNumberFormatLocales
    );
...
function register(moduleId, binding) {
  if (!isHijackableModuleId(moduleId)) {
    throw new Error(
      `Cannot register for relative/absolute file paths; no leading '.' or '/' allowed (was given ${moduleId})`
    );
  }

  if (redirects.has(moduleId)) {
    Ti.API.warn(
      `Another binding has already registered for module id: '${moduleId}', it will be overwritten...`
    );
...
class Color {
  ...
  static fromSemanticColorsEntry(entry) {
    ...
    if (HEX_8_REGEX.exec(color)) {
      if (hadAlpha) {
        throw new Error(
          `Color ${entry} had an explicit alpha value AND a hex value containing alpha. Use one or the other.`
        );
      }
...

{
  ...
  UI.fetchSemanticColor = function fetchSemanticColor(colorName) {
    if (!colorset) {
      try {
        const colorsetFile = Ti.Filesystem.getFile(
...
function _addListener(emitter, eventName, listener, prepend) {
  ...
  if (max > 0 && length > max) {
    const w = new Error(
      `Possible EventEmitter memory leak detected. ${length} ${eventName} listeners added. Use emitter.setMaxListeners() to increase limit`
    );
...
class EventEmitter {
  ...
  removeAllListeners(eventName) {
    ...
    if (eventName === undefined) {
      ...
      const names = Object.keys(this._eventsToListeners).filter(
        (name) => name !== "removeListener"
      );
...
function assertArgumentType(arg, name, typename) {
  ...
  if (type !== typename.toLowerCase()) {
    throw new TypeError(
      `The "${name}" argument must be of type ${typename}. Received type ${type}`
    );
  }
}
...
process$1.hasUncaughtExceptionCaptureCallback = () =>
...
process$1.setUncaughtExceptionCaptureCallback = (fn) => {
  ...
  if (uncaughtExceptionCallback !== null) {
    throw new Error(
      "`process.setUncaughtExceptionCaptureCallback()` was called while a capture callback was already active"
    );
  }
...
function basename(separator, filepath, ext) {
  ...
  if (
    lastCharCode === FORWARD_SLASH ||
    (!isPosix && lastCharCode === BACKWARD_SLASH)
  ) {
    endIndex--;
  ...
  if (isPosix) {
    ...
    if (
      ...
      filepath.charAt(1) === ":" &&
      isWindowsDeviceName(filepath.charCodeAt(0))
    ) {
      return "";
    }
...
function resolve(separator, paths) {
  ...
  if (normalized.charAt(normalized.length - 1) === separator) {
    ...
    if (
      ...
      normalized.length === 3 &&
      normalized.charAt(1) === ":" &&
      isWindowsDeviceName(normalized.charCodeAt(0))
    ) {
      return normalized;
...
function parse(separator, filepath) {
  ...
  if (
    length > 1 &&
    isWindowsDeviceName(firstCharCode) &&
    filepath.charAt(1) === ":"
  ) {
    if (length > 2) {
      // is it like C:\\?
...
function format$1(separator, pathObject) {
  ...
  const base =
    pathObject.base || `${pathObject.name || ""}${pathObject.ext || ""}`; // append base to root if `dir` wasn't specified, or if
  // dir is the root
...
const util = {
  ...
  log: (string) => {
    ...
    const time = `${date.getHours().toString().padStart(2, "0")}:${date
      .getMinutes()
      .toString()
      .padStart(2, "0")}:${date.getSeconds().toString().padStart(2, "0")}`; // Produces output like: "21 Feb 10:04:23 - message"

    console.log(
      `${date.getDate()} ${MONTHS[date.getMonth()]} ${time} - ${string}`
    );
  },
...
class AssertionError extends Error {
  constructor(options) {
    let { actual, expected, message, operator } = options;
...
function compareMaps(actual, expected, strictness, references) {
  ...
  for (const [key, value] of actual) {
    if (typeof key === "object" && key !== null) {
      ...
      if (
        expected.has(key) &&
        deepEqual(value, expected.get(key), strictness, references)
      ) {
  ...
  for (const [expectedKey, expectedValue] of expected) {
    ...
    if (
      ...
      !(typeof expectedKey === "object" && expectedKey !== null)
    ) {
      return false;
    ...
    for (const key of looseChecks) {
      ...
      if (
        deepEqual(key, expectedKey, strictness, references) &&
...
function deepEqual(actual, expected, strictness, references) {
  ...
  if (util.types.isRegExp(actual)) {
    ...
    if (
      util.types.isNumberObject(actual) &&
      (!util.types.isNumberObject(expected) ||
        !Object.is(
          Number.prototype.valueOf.call(actual),
          Number.prototype.valueOf.call(expected)
        ))
    ) {
      return false;
    } else if (
      util.types.isStringObject(actual) &&
      (!util.types.isStringObject(expected) ||
        String.prototype.valueOf.call(actual) !==
          String.prototype.valueOf.call(expected))
    ) {
      return false;
    } else if (
      util.types.isBooleanObject(actual) &&
      (!util.types.isBooleanObject(expected) ||
        Boolean.prototype.valueOf.call(actual) !==
          Boolean.prototype.valueOf.call(expected))
    ) {
      ...
      util.types.isSymbolObject(actual) &&
      (!util.types.isSymbolObject(expected) ||
        Symbol.prototype.valueOf.call(actual) !==
          Symbol.prototype.valueOf.call(expected))
    ) {
      return false;
    } // continue on to check properties...
  ...
  if (strictness === STRICTNESS.Strict) {
    ...
    if (actualSymbols.length > 0) {
      ...
      for (const key of actualSymbols) {
        const actualIsEnumerable = Object.prototype.propertyIsEnumerable.call(
          actual,
          key
        );
        const expectedIsEnumerable = Object.prototype.propertyIsEnumerable.call(
          expected,
          key
        );
...
async function executePromise(fn) {
  ...
  if (fnType === "function") {
    ...
    if (!isPromiseLike(promise)) {
      throw new TypeError(
        `Expected instanceof Promise to be returned from the "fn" function but got ${typeof promise}`
      );
    }
  } else {
    if (!isPromiseLike(fn)) {
      throw new TypeError(
        `The "fn" argument must be of type Function or Promise. Received type ${fnType}`
      );
    }
...
function unsupportedNoop(moduleName, name) {
  return () => {
    ...
    oneTimeWarning(
      fqn,
      `"${fqn}" is not supported yet on Titanium and uses a no-op fallback.`
    );
    return undefined;
  };
}
...
const permissionDenied = (syscall, path) =>
  makeError("EACCES", "permission denied", -13, syscall, path);

const noSuchFile = (syscall, path) =>
  makeError("ENOENT", "no such file or directory", -2, syscall, path);

const fileAlreadyExists = (syscall, path) =>
  makeError("EEXIST", "file already exists", -17, syscall, path);

const notADirectory = (syscall, path) =>
  makeError("ENOTDIR", "not a directory", -20, syscall, path);

const directoryNotEmpty = (syscall, path) =>
  makeError("ENOTEMPTY", "directory not empty", -66, syscall, path);

const illegalOperationOnADirectory = (syscall, path) =>
  makeError("EISDIR", "illegal operation on a directory", -21, syscall, path);
...
fs.chmod = (path, mode, callback) =>
  asyncUnsupportedNoop("fs", "chmod", callback);

fs.chmodSync = unsupportedNoop("fs", "chmodSync");

fs.chown = (path, uid, gid, callback) =>
  asyncUnsupportedNoop("fs", "chown", callback);
...
fs.fchmod = (fd, mode, callback) =>
  asyncUnsupportedNoop("fs", "fchmod", callback);

fs.fchmodSync = unsupportedNoop("fs", "fchmodSync");

fs.fchown = (fd, uid, gid, callback) =>
  asyncUnsupportedNoop("fs", "fchown", callback);

fs.fchownSync = unsupportedNoop("fs", "fchownSync");

fs.fdatasync = (fd, callback) =>
  asyncUnsupportedNoop("fs", "fdatasync", callback);
...
fs.mkdtempSync = (prefix, options) => {
  ...
  throw new Error(
    `Failed to create a unique directory name with prefix ${prefix}`
  );
};
/**
...
fs.openSync = (path, flags = "r", _mode = 0o666) => {
  ...
  if (!tiFile.exists()) {
    ...
    oneTimeWarning(
      "fs.openSync.mode",
      "fs.openSync's mode parameter is unsupported in Titanium and will be ignored"
    );
    ...
    // file/dir exists...
    if (
      (flags.charAt(0) === "w" || flags.charAt(0) === "a") &&
      tiFile.isDirectory()
    ) {
  ...
  if (tiMode === undefined) {
    ...
    const err = new TypeError(
      `The value "${String(flags)}" is invalid for option "flags"`
    );
...
fs.read = (fd, buffer, offset, length, position, callback) => {
  ...
  if (position !== null) {
    oneTimeWarning(
      "fs.readSync.position",
      "fs.readSync's position argument is unsupported by Titanium and will be treated as null"
    );
  }
...
fs.readdirSync = (filepath, options) => {
  ...
  if (options.withFileTypes === true) {
    ...
    oneTimeWarning(
      "fs.readdir's options.withFileTypes is unsupported by Titanium and strings will be returned"
    );
...
fs.readFileSync = (path, options) => {
  ...
  const wasFileDescriptor = typeof path === "number";
  const fileDescriptor = wasFileDescriptor
    ? path
    : fs.openSync(path, options.flag); // use default mode
...
fs.renameSync = (oldPath, newPath) => {
  ...
  if (destFile.isFile()) {
    ...
    tempPath = path.join(
      fs.mkdtempSync(path.join(Ti.Filesystem.tempDirectory, "rename-")),
...
fs.utimes = (path, atime, mtime, callback) =>
  asyncUnsupportedNoop("fs", "utimes", callback);
...
fs.writeFileSync = (file, data, options) => {
  ...
  const fileDescriptor = wasFileDescriptor
    ? file
    : fs.openSync(file, options.flag, options.mode); // if data is a string, make it a buffer first
...
function pipeViaWriteStream(srcStream, destStream, callback) {
  Ti.Stream.writeStream(
    srcStream,
    destStream,
    COPY_FILE_CHUNK_SIZE,
    (result) => {
      if (!result.success) {
...
function fetchScriptsFromJson() {
  ...
  try {
    ...
    Ti.API.error(
      `Failed to read "${JSON_FILE_NAME}". Reason: ${error.message}`
    );
  }
...
function fetchScriptsFromResourcesDirectory() {
  const resourceDirectory = Ti.Filesystem.getFile(
    Ti.Filesystem.resourcesDirectory
  );
      ...
      if (file.isDirectory()) {
        ...
        if (fileNameArray) {
          for (let index = 0; index < fileNameArray.length; index++) {
            loadFrom(
              Ti.Filesystem.getFile(file.nativePath, fileNameArray[index])
            );
          }
        }
...
Ti.API.info(
  `${Ti.App.name} ${
    Ti.App.version
  } (Powered by Titanium ${"9.2.1"}.${"19122961e3"})`
